...
def delete_intermediate_files():
    os.remove("AMSA.xlsx")
    os.remove("AMSA1.xlsx")
    os.remove("AMSA2.xlsx")
    os.remove("New_ini_Target.xlsx")
    os.remove("output_Phase.xlsx")
    os.remove("output_spk.xlsx")
...
def update_data_columns(product):
    ...
    if product == 'Akari':
        ...
        #elif product == 'Your_Product_Name':
            #data_columns = {
                #}
...
def productOption():
    ...
    frame.place(relx=frame_x / screen_width, rely=0.35, relwidth=frame_width, relheight=frame_height)  # Adjust rely

    product_combobox = ttk.Combobox(
        frame,
        textvariable=var,
        values=['Select Product', 'Akari', 'Fuji'],  # Adding a placeholder
        font=("Calibri", 12),
        state='readonly'  # Making it read-only
    )
...
def processAMSA():
    ...
    frame = create_frame(0.9, 0.15, 0.45)  # Adjust rely
    ...
    process_button.place(relx=0.67, rely=0.3)

def loadDMCCode():
    frame = create_frame(0.9, 0.15, 0.6)  # Adjust rely
...
def loadCalibSetupFile():
    ...
    frame = create_frame(0.9, 0.15, 0.7)  # Adjust rely
...
def create_frame(width, height, rely):
    screen_width = window.winfo_screenwidth()
