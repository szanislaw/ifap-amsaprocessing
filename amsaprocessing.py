...
def mainWindow():
    ...
    description_label = tk.Label(window, text="Welcome to the Data Analysis App!", font=("Calibri", 16, "bold"))
    description_label.pack(pady=20)
    
    instructions_label = tk.Label(window, text="Instructions: Select a project, upload files, and process data.", font=("Calibri", 12))
...
def processAMSA():
    ...
    edit_button = tk.Button(frame, text='Process Raw Data and GD Selection', command=split_file, font=button_font)
    edit_button.place(relx=0.8, rely=0.25)

def loadDMCCode():
    frame = create_frame(0.9, 0.15, 0.5)

    global file_label_1

    file_label_1 = tk.Label(frame, text=" ")
    file_label_1.place(relx=0.2, rely=0.4)
...
def create_frame(width, height, rely):
    ...
    frame = tk.Frame(window, bd=0, highlightthickness=1, highlightbackground="black", highlightcolor="black")
